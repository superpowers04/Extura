	...
	public static void loadAvatar(Path path, UserData target) {
		...
		async(() -> {
			try {
				...
				{
					Path scriptsPath = IOUtils.getOrCreateDir(FiguraMod.getFiguraDirectory(), "global_scripts");
					CompoundTag newScripts = loadScripts(scriptsPath, scriptsPath);
					if (!newScripts.getAllKeys().isEmpty()) scripts.put("global", newScripts);

					Path preprocessorScriptsPath = IOUtils.getOrCreateDir(FiguraMod.getFiguraDirectory(), "global_scripts/.preprocessor_scripts");
					CompoundTag prenewScripts = loadScripts(preprocessorScriptsPath, preprocessorScriptsPath);
					if (!prenewScripts.getAllKeys().isEmpty()) nbt.put("preprocessors", prenewScripts);
				}
				if (!scripts.getAllKeys().isEmpty()) nbt.put("scripts", scripts);
