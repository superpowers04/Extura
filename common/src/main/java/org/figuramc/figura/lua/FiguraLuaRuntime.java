...
public class FiguraLuaRuntime {
	...
	public Map<String, String> preprocessors = new HashMap<>();
	...
	private Varargs initializeScript(String str){
		return initializeScript(str,true);
	}
	private Varargs initializeScript(String str,boolean callPreprocessor){
		...
		if (src == null) throw new LuaError("Tried to require nonexistent script \"" + name + "\"!");
		if(owner.isHost && callPreprocessor && !preprocessors.isEmpty()){
			String process = owner.preprocessScriptEvent(name,src);
			if(process != "unchanged"){
				scripts.put(name,src = process);
				String[] scriptPath = name.split(".");
				int i = 0;
				int max = scriptPath.length - 2;
				CompoundTag nbt = owner.nbt.getCompound("scripts");
				while(i < max){
					nbt = nbt.getCompound(scriptPath[i]);
					i++;
				}
				nbt.put(scriptPath[max + 1],new ByteArrayTag(src.getBytes(StandardCharsets.UTF_8)));
			}
		}
	...
	public boolean initPreprocessors(ListTag autoScripts) {
		...
		try {
			...
			if (owner.isHost && !preprocessors.isEmpty()) {
				for (String name : preprocessors.keySet()){
					String extName = "EXTURAPREPROCESSOR/" + name;
					scripts.put(extName,preprocessors.get(name));
					initializeScript(extName,false);
				}
			}
	...
	public boolean init(ListTag autoScripts) {
		...
		try {
			if (autoScripts == null) {
				for (String name : scripts.keySet()) initializeScript(name);
			} else {
				for (Tag name : autoScripts) initializeScript(name.getAsString());
			}
		} catch (Exception | StackOverflowError e) {
			error(e);
			return false;
